...
An iterator is very simple conceptually, but is very powerful. You can think of it as a movable cursor that can only move in one direction but also points into a collection of some sort. In our list example above, it would look something like this:
...
println!("{:?}", first); // Some(0). Some just means a non-null value.
...
1. Rust's std::iter crate
2. C#'s System.Linq library

The language-level implementation and these libraries will take care of 90% of the iterator use cases for you and you really shouldn't reinvent the wheel here. Just use the built-in machinery of your language if it is available to you. 

The places where you _do_ want to use an iterator are the following:
- You have state that can be used to derive a list of some other value. For example, I wrote an iterator to slice up a date range by a value. Give a date range and a slice size, I would calculate a series of smaller ranges. (ex. Give 3/2/2021-12/4/2023 and a slice of month, I would generate 3/2/2021-3/31/2021, 4/1/2021-4/30/2021, ..., 12/1/2023-12/4/2023). This lets you calculate that state on-the-fly and avoid synchronization issues between the two pieces of state.
...
## Implementation
The implementation of an iterator tends to be pretty simple. I'm going to appeal back to your introduction to recursion class and show you how to implement the Fibonacci sequence using and iterator. To start with, we need to define a struct that holds onto the current and next numbers in the sequence.
...
- [Iterator()](https://docs.oracle.com/javase/8/docs/api/java/util/Iterator.html) in Java
